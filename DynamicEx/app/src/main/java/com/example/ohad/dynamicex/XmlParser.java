			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (n.getNodeType() == Node.ELEMENT_NODE) {
					...
					if(type.equals("ListenFindGame")){
						...
						lesson.addSlide(newSlide);
					} if (type.equals("OrderGame")){
						XmlParser.Game gameType = XmlParser.Game.ORDER
						int maxNum = e.getElementsByTagName("max_num").item(0).getTextContent();
